                        ...
                        if (entities[i].getComponent('healable').items.map(x => x.item).includes(entities[i].getComponent('inventory').container.getItem(j).typeId)) {
                            try {
                                for (let k = 0; k < Object.keys(entities[i].getComponent('healable').items).length; k++) {
                                    ...
                                    try {
                                        switch (entities[i].getComponent('inventory').container.getItem(j).typeId) {
                                            case "rift:cooked_dodo_meat":
                                                entities[i].runCommandAsync(`effect @s absorption 30 3`)
                                                break
                                            case "rift:cooked_hadrosaur_meat":
                                                entities[i].runCommandAsync(`effect @s resistance 60 3`)
                                                break
                                            case "rift:mixed_meat":
                                                entities[i].runCommandAsync(`effect @s absorption 120`)
                                                entities[i].runCommandAsync(`effect @s regeneration 5 1`)
                                                break
                                            ...
                                            case "rift:enchanted_mixed_meat":
                                                entities[i].runCommandAsync(`effect @s absorption 120 3`)
                                                entities[i].runCommandAsync(`effect @s regeneration 30 1`)
                                                entities[i].runCommandAsync(`effect @s fire_resistance 300`)
                                                entities[i].runCommandAsync(`effect @s resistance 300`)
                                                break
                                    ...
                                    entities[i].runCommandAsync(`event entity @s rift:start_eat_from_inventory_cooldown`)
                                }
                                break
                            }
                            catch (e) {}
