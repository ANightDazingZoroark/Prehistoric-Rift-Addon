...
world.events.tick.subscribe((ev) => {
    if (ev.currentTick % 20 == 0) {
        ...
        mainloop: for (let e = 0; e < entities.length; e++) {
            ...
            if (entities[e].id == 'rift:parasaurolophus' && !entities[e].hasTag('tamed') && !entities[e].hasTag('angeredByFire') && !entities[e].hasTag('baby') && !entities[e].target) {
                for(let x = -12; x < 13; x++) {
                    for (let y = -2; y < 4; y++){
                        for (let z = -12; z < 13; z++) {
                            if (fireBlocks.includes(entities[e].dimension.getBlock(new BlockLocation(Math.trunc(entities[e].location.x+x), Math.trunc(entities[e].location.y+y), Math.trunc(entities[e].location.z+z))).id)) {
            ...
            if (entities[e].id == 'rift:direwolf' && !entities[e].hasTag('tamed') && !entities[e].hasTag('angeredByFire') && entities[e].hasTag('ignited') && !entities[e].hasTag('baby')) {
                for(let x = -12; x < 13; x++) {
                    for (let y = -2; y < 4; y++){
                        for (let z = -12; z < 13; z++) {
                            if (fireBlocks.includes(entities[e].dimension.getBlock(new BlockLocation(Math.trunc(entities[e].location.x+x), Math.trunc(entities[e].location.y+y), Math.trunc(entities[e].location.z+z))).id)) {
